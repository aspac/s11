...
int main ()
{
	...
	apn_rest *nApn_rest;
	bearer_cont *nBearer_cont;
	bearer_cont1 *nBearer_cont1;
	bearer_cont4 *nBearer_cont4;
	ebi *nEbi;
	pti *nPti;
	cause *nCause;
	...
	bzero(&ggsn,sizeof(ggsn));	
	bzero(buffer,sizeof(buffer));
	bzero(buffer,sizeof(buff));
	length=sizeof(sgsn);

	ggsn.sin_family=AF_INET;
	ggsn.sin_addr.s_addr=htonl(INADDR_ANY);
	...
	if(bind(sock,(struct sockaddr *)&ggsn,sizeof(ggsn))==-1){
		perror("\nBind error: ");
		return -1;
	}
	...
	while(1) { 
	if(recvfrom(sock,buffer,sizeof(buffer),0,(struct sockaddr *)&sgsn,&length)==-1){
		perror("\nRecvFrom error: ");
		return -1;
	}
	...
	if(k->m_type == 32)
	{
		...
		nBearer_cont->_cause.flags1=0x00;
  	        ...
  	        paclength=4+ntohs(g->m_length);
                if(sendto(sock,buff,paclength,0,(struct sockaddr *)&sgsn,sizeof(sgsn  ))==-1){
                          perror("\nSend: ");
                          return -1;
                }		
